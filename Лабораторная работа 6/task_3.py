...
def to_csv_file(filename, headers, rows, delimiter =",", new_line="\n"):
    with open(filename, "w") as output_f:
        output_f.write(delimiter.join(headers) + new_line)  # запись в файл с преобразованием списка headers в строку
        output_f.writelines(delimiter.join(row) + new_line for row in rows) # запись в файл последовательности строк
    return
